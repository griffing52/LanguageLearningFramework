 ...
 * @param {string} extension file extension such as 'json' or 'txt'
 */
    const saveFile = (content, currPage, extension) => {
    ...
    const downloadAllAndMoveOn = () => {
        ...
        saveFile(JSON.stringify(phrases), '_' + currPage, 'json')
        saveFile(phrases.map(a => `${formatText(a.phrase)}=${a.translation}\n`).reduce((acc, elm) => acc + elm), currPage, 'txt')
